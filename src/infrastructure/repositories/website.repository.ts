...
import {
  ...
  WebsiteUpdate,
  WebsiteWithLocation,
} from "@/src/entities/models/website";
...
export class WebsiteRepository implements IWebsiteRepository {
  async create(website: WebsiteInsert): Promise<Website> {
    return await startSpan({ name: "WebsiteRepository > create" }, async () => {
      try {
  ...
  async update(website: WebsiteUpdate): Promise<Website> {
    return await startSpan({ name: "WebsiteRepository > update" }, async () => {
      try {
  ...
  async delete(id: string): Promise<Website> {
    return await startSpan({ name: "WebsiteRepository > delete" }, async () => {
      try {
        // Delete related records in GoogleKeywordTrackerTool --> also check 'insert' google-keyword-tracker.repository.ts
        await db.googleKeywordTrackerTool.deleteMany({
          where: {
            websiteId: id,
          },
        });
