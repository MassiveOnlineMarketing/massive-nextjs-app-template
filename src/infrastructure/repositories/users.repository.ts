...
export class UsersRepository implements IUsersRepository {
  async create(email: string, password: string, name: string): Promise<User> {
    return await startSpan({ name: "UsersRepository > create" }, async () => {
      try {
  ...
  async getById(id: string): Promise<User | null> {
    return await startSpan({ name: "UsersRepository > getById" }, async () => {
      try {
        const user = await db.user.findUnique({
          where: { id },
        });
  ...
  async update(data: any, userId: string): Promise<User> {
    return await startSpan({ name: "UsersRepository > update" }, async () => {
      try {
