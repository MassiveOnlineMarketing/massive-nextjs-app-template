...
const MainSideMenu = () => {
  ...
  return (
    <div className='w-[80px] flex flex-col h-screen pl-[18px] pr-[10px] pb-4 group/side-bar hover/side-bar:w-[328px] transition-all duration-300 '>


        <div className='group-hover/side-bar:w-full h-[76px] flex'>
          <LogoWithHover />
        </div>
        <div className='group-hover/side-bar:w-full h-[88px] flex items-center' >
          <WebsiteSelectionButtonHover websites={websites} />
        </div>

        <div className='group-hover/side-bar:w-full  flex flex-col'>
      ...
      <div className='group-hover/side-bar:w-full  mt-auto'>
...
const NavLabelWithHover = ({ children }: { children: React.ReactNode }) => {
  return (
    <p className='text-xs text-nowrap group-hover/side-bar:theme-t-n px-3 py-1 text-base-50 group-hover/side-bar:block hidden'>{children}</p>
  )
}
...
const NavItemWithHover = ({ item }: { item: NavigationItem }) => {
  return (
    <Link
      href={item.link}
      className={cn(
        'p-3 flex gap-2 items-center h-[50px] w-[50px] group-hover/side-bar:w-full',
        item.active ? 'theme-bg-w group-hover/side-bar:rounded-none group-hover/side-bar:border-x theme-b-p ' : '',
        item.active ? ' group-hover/side-bar:border-x theme-b-p ' : '',
        item.active ? 'group-hover/side-bar:before:border-0 group-hover/side-bar:after:border-0 molecule rounded-[8px] before:rounded-[8px] after:rounded-[12px] before:left-0' : ''
      ...
      <p
        className={cn(
          'text-nowrap',
          item.active ? 'theme-t-p' : 'theme-t-n',
          'group-hover/side-bar:block hidden'
        )}
      >{item.label}</p>
    </Link >
  )
...
const LogoWithHover = () => {
  return (
    <div className='mt-auto mb-2 flex items-center gap-2.5'>
      ...
      <div className='hidden group-hover/side-bar:block'><MassiveTextDash /></div>
    </div>
  )
}
